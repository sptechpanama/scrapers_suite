...
import sys, re, time, unicodedata, json
...
CFG = {
    ...
    "sheets_data": ["cl_prog_sin_ficha", "cl_prog_sin_requisitos", "cl_prog_con_ct", "cl_prog_419_sfd", "cl_prioritarios"],
    "sheet_desc": "cl_descartes",
    "sheet_prio": "cl_prioritarios",
    "sheet_ct_rir": "cl_prog_ct_rir",
    "sheet_419_sfd": "cl_prog_419_sfd",
...
def ensure_sheet_exists(title: str, rows: int = 4000, cols: int = 30) -> bool:
    ...
    try:
        _exec_with_retry(
            f"addSheet {title}",
            lambda: GSVC.spreadsheets().batchUpdate(
                spreadsheetId=SSID,
...
def purge_all():
    for s in ["cl_prog_sin_ficha","cl_prog_sin_requisitos","cl_prog_con_ct",CFG["sheet_419_sfd"],CFG["sheet_ct_rir"],"cl_prioritarios","cl_descartes"]:
        purge_by_fecha(s)
...
def main():
    ...
    listing = collect_listing(PT, CFG["css_links"], LOG, expected_state="Programada")
    listing = recover_listing(listing, PT, "clrir", LOG, normalize_url=normalize_url)
    links = listing["links"]
    listing_complete = listing["complete"]
    coverage = DetailCoverage(DATA_DIR / "scrape_coverage", "clrir", listing)
    ...
    try:
        ...
        LOG("SHEETS", f"cl_prog_sin_requisitos: metadata pendiente: {type(exc).__name__}: {exc}")
    ...
    if not nuevos:
        move_rows_by_checkbox(['cl_prog_sin_ficha','cl_prog_sin_requisitos','cl_prog_con_ct',CFG["sheet_419_sfd"],CFG["sheet_ct_rir"]], CFG["sheet_prio"], "Prioritario")
        move_rows_by_checkbox(['cl_prog_sin_ficha','cl_prog_sin_requisitos','cl_prog_con_ct',CFG["sheet_419_sfd"],CFG["sheet_ct_rir"]], CFG["sheet_desc"], "Descartar")
        for sh in ['cl_prog_sin_ficha','cl_prog_sin_requisitos','cl_prog_con_ct',CFG["sheet_419_sfd"],CFG["sheet_ct_rir"]]:
            reset_checkboxes(sh)
    ...
    prog_419_rows, prog_419_cols = append_df(CFG["sheet_419_sfd"], df_prepare(datos_419_sfd))
    ...
    for sh in ['cl_prog_sin_ficha','cl_prog_sin_requisitos','cl_prog_con_ct',CFG["sheet_419_sfd"],CFG["sheet_ct_rir"],'cl_prioritarios','cl_descartes']:
        purge_by_fecha(sh)
    for sh in ['cl_prog_sin_ficha','cl_prog_sin_requisitos','cl_prog_con_ct',CFG["sheet_419_sfd"],CFG["sheet_ct_rir"]]:
        reset_checkboxes(sh)
        update_fechas_sheet(sh)
    ...
    for sheet_name, rows_out, cols_out in [
        ...
        (CFG["sheet_419_sfd"], prog_419_rows, prog_419_cols),
    ]:
