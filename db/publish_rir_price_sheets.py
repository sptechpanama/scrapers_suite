...
from orquestador.google_transport import build_sheets_service, close_service
...
def publish(
    *,
    ...
    if not rows or len({str(row[0]) for row in rows}) != len(rows):
        raise ValueError("No se publica una tabla de precios vacía o con fichas duplicadas.")
    ...
    try:
        return publish_verified(service, spreadsheet_id, rows)
    finally:
        close_service(service)


def publish_verified(service: Any, spreadsheet_id: str, rows: list[list[Any]]) -> dict[str, int]:
    _ensure_sheets(service, spreadsheet_id)
    historical_values = [HISTORICAL_HEADERS, *rows]
    before = _call_with_backoff(lambda: service.spreadsheets().values().batchGet(
        spreadsheetId=spreadsheet_id,
        ranges=[f"'{HISTORICAL_SHEET}'!A:P", f"'{RESEARCH_SHEET}'!1:1"],
        valueRenderOption="UNFORMATTED_VALUE",
    ).execute())["valueRanges"]
    old_values = before[0].get("values", [])
    existing = (before[1].get("values") or [[]])[0]
    if any(not str(value or "").strip() for value in existing):
        raise ValueError("El encabezado de investigación tiene huecos; se conserva sin desplazar columnas.")
    research_headers = merge_research_headers(existing)
    width = len(HISTORICAL_HEADERS)
    height = max(len(old_values), len(historical_values))
    padded = historical_values + [[""] * width for _ in range(height - len(historical_values))]
    target = f"'{HISTORICAL_SHEET}'!A1:{_column_letter(width)}{height}"
    updates = [{"range": target, "values": padded}]
    if research_headers != existing:
        updates.append({"range": f"'{RESEARCH_SHEET}'!A1:{_column_letter(len(research_headers))}1", "values": [research_headers]})
    # One atomic write replaces prices and clears only obsolete trailing cells.
    # Never clear a sheet before the new data has been accepted by Google.
    _call_with_backoff(lambda: service.spreadsheets().values().batchUpdate(
        spreadsheetId=spreadsheet_id,
        body={"valueInputOption": "RAW", "data": updates},
    ).execute())
    verified = _call_with_backoff(lambda: service.spreadsheets().values().get(
        spreadsheetId=spreadsheet_id, range=target, valueRenderOption="UNFORMATTED_VALUE",
    ).execute()).get("values", [])
    normalized = [list(row) + [""] * (width - len(row)) for row in verified]
    while normalized and not any(value != "" for value in normalized[-1]):
        normalized.pop()
    if normalized != historical_values:
        raise RuntimeError("La lectura de verificación de precios no coincide con la publicación.")
    return {
